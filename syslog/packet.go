...
// ALEX: This has been modified to deal with RFC5424 structured data
// - - - : hostname appnameTag pid msgid sd
func (p Packet) Generate(max_size int) string {
	...
	if max_size == 0 {
		return msg //fmt.Sprintf("<%d>1 %s %s %s - - - %s", p.Priority(), ts, p.Hostname, p.Tag, p.cleanMessage())
	} else {
		//msg := fmt.Sprintf("<%d>1 %s %s %s - - - %s", p.Priority(), ts, p.Hostname, p.Tag, p.cleanMessage())
		if len(msg) > max_size {
